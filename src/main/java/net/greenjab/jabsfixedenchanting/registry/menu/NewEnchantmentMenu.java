        ...
        if (container == this.enchantSlots) {
            ...
            if (!itemStack.isEmpty() && itemStack.isEnchantable()) {
                this.access.execute( (level, pos) -> {
                    ...
                    int bookShelfCount = JabsFixedEnchantmentHelper.countAccessibleBookshelves(level, pos);
                    int power = (int)((JabsFixedEnchantmentHelper.POWER_WHEN_MAX_LEVEL-1) * bookShelfCount / 15f + 1);
                    ...
                    for (int slot = 0; slot < 3; slot++) {
                        ...
                        if (!enchantments.isEmpty()) {
                            ...
                            for (EnchantmentInstance entry : enchantments) {
                                enchantmentPower += JabsFixedEnchantmentHelper.getEnchantmentPower(entry.enchantment(), entry.level());
                            }
                            int cap = JabsFixedEnchantmentHelper.getEnchantmentCapacity(itemStack);
    ...
    private List<EnchantmentInstance> getEnchantmentList(final RegistryAccess access, final ItemStack itemStack, final int slot, final int enchantmentCost) {
        ...
        if (tag.isEmpty()) {
            ...
            this.access.execute((world, tablePos) -> {
                int bookShelfCount = JabsFixedEnchantmentHelper.countAccessibleBookshelves(world, tablePos);
                for (int i=0; i<bookShelfCount; i++) {
                    int randomIndex = this.random.nextInt(EnchantingTableBlock.BOOKSHELF_OFFSETS.size());
                    if (!JabsFixedEnchantmentHelper.canAccessBlock(world, tablePos, EnchantingTableBlock.BOOKSHELF_OFFSETS.get(randomIndex), Blocks.CHISELED_BOOKSHELF)) continue;
            ...
            enchPower.addAndGet(JabsFixedEnchantmentHelper.getEnchantmentPower(randomEntry.enchantment(), randomEntry.level()));
            ...
            for (ItemStack chosenStack : chosenItemStacks) {
                ...
                for (Object2IntMap.Entry<Holder<Enchantment>> entry : bookEnchantments.entrySet()) {
                    ...
                    if ((enchPower.get() + JabsFixedEnchantmentHelper.getEnchantmentPower(registryEntry, level)) <= 0) continue;
                ...
                if (!enchantments2.isEmpty()) {
                    ...
                    enchantments2.forEach((enchantment, level) -> {
                        if (i[0] == rand) {
                            enchPower.addAndGet(JabsFixedEnchantmentHelper.getEnchantmentPower(enchantment, level));
            ...
            enchantments.forEach((enchantment, level) -> {
                if (!itemStack.is(Items.BOOK) || enchantment.value().getMaxLevel()>1) enchantmentsResult.add(new EnchantmentInstance(enchantment, (JabsFixedEnchanting.SERVER.getGameRules().get(GameRuleRegistry.SUPER_ENCHANT_CHANCE)>0&&itemStack.is(ItemTags.PIGLIN_LOVED)&&enchantment.value().getMaxLevel()!=1)?level+1:level));
            });
